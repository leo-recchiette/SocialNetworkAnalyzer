...
export default class NeoVis {
	...
	_network_events = (eventData) => {
		for (let nodeId of eventData.nodes) {
			const node = this._data.nodes.get(nodeId);
			const nodeConfig = this._config && this._config.labels && this._config.labels[node.originalLabels[0]];
			if (nodeConfig && typeof nodeConfig.clickEvent === 'function') {
				nodeConfig.clickEvent({
					properties: node.originalProperties,
					labels: node.originalLabels,
		...
		for (let edgeId of eventData.edges) {
			...
			const edgeConfig = this._config && this._config.relationships && this._config.relationships[edge.originalType];
			if (edgeConfig && typeof edgeConfig.clickEvent === 'function') {
				edgeConfig.clickEvent({
		...
		session
			...
			.subscribe({
				...
				onCompleted: async () => {
					...
					// );
					if (this._network) {
						this._network.off('click', this._network_events);
					}
					...
					this._network.on('click', this._network_events);
				},
				onError: (error) => {
